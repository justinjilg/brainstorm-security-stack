...
async function main() {
  ...
  console.log(`\n  Sprint 1 progress: ${completed.length}/${SPRINT_1_TASKS.length} tasks complete`);
  ...
  if (arg === "next" || !arg) {
    task = getNextTask();
    if (!task) {
      console.log("  All Sprint 1 tasks complete!");
      process.exit(0);
    }
  } else {
    const num = parseInt(arg, 10);
    task = SPRINT_1_TASKS.find(t => t.number === num) ?? null;
    if (!task) {
  ...
  const nextTask = SPRINT_1_TASKS.find(t => t.number === task!.number + 1);
  if (nextTask) {
